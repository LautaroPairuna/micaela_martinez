    ...
    for (const modKey of Object.keys(prog)) {
      ...
      for (const lessonKey of Object.keys(mod)) {
        ...
        const completed = !!(
          ...
          'completed' in lessonData &&
          (lessonData as { completed: boolean }).completed
        );
  ...
  */
  ...
  const logProgressSummary = useCallback(
    (params: {
      ...
      try {
        const completedKeys = Object.keys(params.serverProgress).filter((k) => params.serverProgress[k]);
        ...
        const sampleModules = (params.modulesToUse || []).slice(0, 3).map((m) => ({
          id: m?.id,
          lessons: (m?.lecciones || []).slice(0, 5).map((l) => {
            const key = getLessonProgressKey ? getLessonProgressKey(String(m?.id), String(l?.id)) : `${m?.id}-${l?.id}`;
            const isDone = !!(params.serverProgress[key] || (params.lessonProgress && params.lessonProgress[key]));
  ...
  const realTimeProgress = useMemo(() => {
    const modulesToUse =
      course?.modulos && course.modulos.length > 0 ? course.modulos : courseModules && courseModules.length > 0 ? courseModules : [];
    ...
    modulesToUse.forEach((module: any) => {
      if (module.lecciones?.length) {
        module.lecciones.forEach((lesson: any) => {
          ...
          const isCompletedNow = hasClientState ? !!lessonProgress[progressKey] : !!serverProgress[progressKey];
          if (isCompletedNow) {
            completedLessons++;
          }
        });
  ...
  const isCompleted =
    enrollment.estado === 'DESACTIVADA' ||
    progressPct >= 100 ||
    (realTimeProgress.totalLessons > 0 && realTimeProgress.completedLessons >= realTimeProgress.totalLessons);
      ...
      <CardBody className="relative flex flex-col gap-3 flex-1 p-4 sm:p-5 lg:p-6">
        ...
        <div className="space-y-2 pt-2">
          <div className="flex items-center justify-between text-xs uppercase tracking-wider font-bold text-zinc-500">
            <span>Avance</span>
            <span className={isCompleted ? 'text-green-400' : 'text-white'}>{isCompleted ? '100%' : `${Math.round(progressPct)}%`}</span>
          </div>

          <div className="w-full bg-zinc-800 rounded-full h-1.5 overflow-hidden">
            <div
              className={`h-full shadow-[0_0_10px_rgba(var(--gold-rgb),0.3)] transition-all duration-700 ease-out ${
                isCompleted ? 'bg-green-500 shadow-green-500/30' : 'bg-gradient-to-r from-[var(--gold)] to-[#b88a44]'
              }`}
