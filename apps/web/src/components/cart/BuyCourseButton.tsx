...
export function BuyCourseButton({
  ...
  const handleAddCourse = async () => {
    try {
      // ✅ IMPORTANTÍSIMO: evitar mezcla con productos persistidos
      clear();
  ...
  return (
    <button
      ...
      className={`group relative overflow-hidden rounded-2xl bg-gradient-to-r from-[var(--gold)] via-[var(--gold)] to-[var(--gold)]/90 px-8 py-4 font-bold text-black shadow-xl transition-all duration-300 hover:shadow-2xl hover:scale-[1.02] active:scale-[0.98] ${
        className || ''
      }`}
    >
      ...
      <div className="relative flex items-center justify-center gap-3">
        ...
        <span className="text-lg tracking-wide">COMPRAR AHORA</span>
        <svg
