...
export class MpPaymentService {
  ...
  async getPayment(paymentId: string | number): Promise<MercadoPagoPaymentResponse> {
    try {
      const response = await this.payment.get({ id: String(paymentId) });
      return response as MercadoPagoPaymentResponse;
    } catch (error: any) {
      const msg =
        error?.cause?.[0]?.message ||
  ...
  // Compatibilidad con código anterior que usa createPayment
  async createPayment(input: any, idemKey?: string) {
    // Adaptador para el método anterior
