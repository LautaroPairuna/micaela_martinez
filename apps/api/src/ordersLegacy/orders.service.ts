...
export class OrdersService {
  ...
  private async handlePaymentWebhook(
    ...
    // 2) Upsert pago
    const mpId = String(paymentDetails.id);

    // Usamos la nueva tabla 'Pago' en lugar de la eliminada 'pagoSuscripcion'
    await this.prisma.pago.upsert({
      where: {
        provider_kind_mpId: {
          provider: 'MERCADOPAGO',
          kind: 'SUBSCRIPTION_PAYMENT', // Asumimos pago de sub o one-off según orden
          mpId,
        },
      },
      create: {
