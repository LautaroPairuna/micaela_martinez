...
-- DropIndex
DROP INDEX `item_carrito_curso_id_fkey` ON `item_carrito`;

-- DropIndex
DROP INDEX `item_carrito_producto_id_fkey` ON `item_carrito`;
...
-- AlterTable
ALTER TABLE `slider` ADD COLUMN `cta_primario_href` VARCHAR(255) NULL,
    ADD COLUMN `cta_primario_texto` VARCHAR(80) NULL,
    ADD COLUMN `cta_secundario_href` VARCHAR(255) NULL,
    ADD COLUMN `cta_secundario_texto` VARCHAR(80) NULL,
    ADD COLUMN `descripcion` VARCHAR(600) NULL,
    ADD COLUMN `etiqueta` VARCHAR(80) NULL,
    ADD COLUMN `subtitulo` VARCHAR(255) NULL;

-- CreateIndex
CREATE INDEX `slider_orden_idx` ON `slider`(`orden`);
