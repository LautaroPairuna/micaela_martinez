-- AlterTable
ALTER TABLE `inscripcion` ADD COLUMN `subscription_active` BOOLEAN NULL,
    ADD COLUMN `subscription_end_date` DATETIME(3) NULL,
    ADD COLUMN `subscription_id` VARCHAR(191) NULL,
    ADD COLUMN `subscription_order_id` INTEGER NULL;

-- AlterTable
ALTER TABLE `orden` ADD COLUMN `suscripcion_proximo_pago` DATETIME(3) NULL;

-- CreateIndex
CREATE INDEX `inscripcion_subscription_order_id_idx` ON `inscripcion`(`subscription_order_id`);

-- CreateIndex
CREATE INDEX `inscripcion_subscription_id_idx` ON `inscripcion`(`subscription_id`);

-- CreateIndex
CREATE INDEX `inscripcion_subscription_end_date_idx` ON `inscripcion`(`subscription_end_date`);

-- CreateIndex
CREATE INDEX `inscripcion_subscription_active_idx` ON `inscripcion`(`subscription_active`);

-- CreateIndex
CREATE INDEX `orden_suscripcion_proximo_pago_idx` ON `orden`(`suscripcion_proximo_pago`);
