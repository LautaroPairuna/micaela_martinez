import { PrismaClient, NivelCurso, EstadoInscripcion, EstadoOrden, TipoItemOrden, TipoLeccion, } from '@prisma/client';
const prisma = new PrismaClient();
const json = (v) => v;
const upsertRoleBySlug = (slug, name) => prisma.role.upsert({
    where: { slug },
    update: { name },
...
const upsertUserByEmail = async (email, data) => {
    await prisma.usuario.upsert({
        where: { email },
        update: { nombre: data.nombre, passwordHash: data.passwordHash, emailVerificadoEn: data.emailVerificadoEn ?? undefined },
        create: { ...data, email },
    });
    const u = await prisma.usuario.findUnique({ where: { email }, select: { id: true } });
    if (!u)
        throw new Error(`No se pudo obtener usuario ${email}`);
    return u.id;
};
const upsertCategoriaBySlug = (slug, nombre, orden = 0, parentId) => prisma.categoria.upsert({
    where: { slug },
...
const upsertMarcaBySlug = (slug, nombre, orden = 0) => prisma.marca.upsert({
    where: { slug },
...
const upsertProductoGetId = async (p) => {
    ...
    if (!prod)
        throw new Error(`No se pudo obtener producto ${p.slug}`);
    return prod.id;
};
const upsertCursoGetId = async (c) => {
    ...
    if (!curso)
        throw new Error(`No se pudo obtener curso ${c.idSlug}`);
    return curso.id;
};
async function main() {
    ...
    await prisma.productoImagen.upsert({
        ...
        if (!exists)
            await prisma.productoImagen.create({ data: { productoId: prodId_crema, archivo: 'crema1.jpg', alt: 'Crema Hidratante - foto 1', orden: 0 } });
    });
    const ensureImg = (productoId, archivo, alt) => prisma.productoImagen.upsert({
        ...
        if (!e)
            await prisma.productoImagen.create({ data: { productoId, archivo, alt, orden: 0 } });
    });
    ...
    const resetCurso = async (cursoId) => {
    ...
    const bootstrapCurso = async (cursoId, titulo1, titulo2) => {
